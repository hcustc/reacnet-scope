...
def test_species_catalog_ignores_excluded_moname_evidence(tmp_path: Path) -> None:
...
def test_event_viewer_exports_frames_trajectory_and_viewer_helpers(
    tmp_path: Path,
    monkeypatch,
) -> None:
    viewer = {
    ...
    monkeypatch.setenv("REACNET_SCOPE_DEPLOYMENT_MODE", "local")
    monkeypatch.setenv("REACNET_SCOPE_OVITO_EXECUTABLE", str(executable))
    monkeypatch.setattr(
        svc.subprocess,
        "Popen",
    ...
    result = svc.launch_event_in_ovito(viewer)

    assert result["pid"] == 8123
    assert launched[0][0] == str(executable.resolve())
    assert Path(result["trajectory"]).read_text(encoding="utf-8") == trajectory


def test_remote_mode_never_attempts_to_detect_or_launch_ovito(monkeypatch) -> None:
    monkeypatch.setenv("REACNET_SCOPE_DEPLOYMENT_MODE", "remote")

    capability = svc.ovito_launch_capability()
