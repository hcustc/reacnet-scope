...
from reacnet_scope.indexes import TrajectoryIndexStore
from reacnet_scope.event_index import EVENT_EVIDENCE_STORE
from reacnet_scope import services as dash_services
from reacnet_scope.queries import read_trajectory_requested_frame_blocks
...
class OnlineIndexContractTests(unittest.TestCase):
    ...
    def test_browser_snapshot_never_opens_reacnet_source_artifacts(self) -> None:
        ...
        # .moname and .route are excluded production inputs.
        self.assertEqual(snapshot["datasets"][0]["score"], len(source_paths) - 2)
    ...
    def test_evolution_and_intermediate_queries_never_open_species_source(self) -> None:
        ...
        def guarded_open(file, *args, **kwargs):
            if os.path.abspath(os.fspath(file)) == os.path.abspath(species):
                raise AssertionError("online query opened raw Species source")
