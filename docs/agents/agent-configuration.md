...
项目技能保留默认自动匹配，也可显式引用；`agents/openai.yaml` 提供中文显示信息与
...
| 技能 | 输入和完成结果 | 边界 |
| --- | --- | --- |
| [reacnet-analysis-usage](../../.agents/skills/reacnet-analysis-usage/SKILL.md) | 已有 RNG 数据与分析问题 → 正确的目标对应、证据解释和多来源比较 | 不默认重跑 RNG、修改软件或把行为差异判断为模型优劣 |
...
普通数据分析先用 usage；它不替代领域契约，也不自动启动开发流程。
...
```text
使用 $reacnet-analysis-usage 查找目标物种并比较两份 RNG 数据的反应行为。
...
加入数据分析使用技能后共 45 个仓库技能。下面是任务路由建议，不是另一套启用配置：
...
| 请求 | 应选择/读取 | 应观察到的行为 |
| --- | --- | --- |
| miso=1 数据中按目标 SMILES 未命中 | usage | 查找对应代表/连接骨架；不直接判断未生成或要求改为 miso=0 |
| 比较两代模型的 MD 结果 | usage | 同口径比较反应行为；物种对应与来源明确，准确性结论需要独立参考 |
| 增加多来源对比界面 | 按改动选择 analysis + Dash | 复用现有分析，不能因使用注意事项扩展成实验管理系统 |
