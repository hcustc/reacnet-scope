...
可切换全部返回路线。点击反应菱形或连线展开步骤证据；完整事件表、精确标签和实际键图分别按需展开。
搜索截断、代表身份限制及连续历史尚需独立检查的提示保留在结果旁。正常就绪不显示准备横幅或入口，
缺证据及降级状态保留原因和准备入口。

物种检索、质量检索的结构列表与物种详情依次占用结果区；详情使用完整结果宽度显示结构图和统计，不在窄侧栏中压缩。进入详情聚焦关闭按钮，Escape 或关闭按钮返回前一阶段；再次点击同一行可以重开。窄屏查询条件换行、详情内容纵向排列，表格在自身区域滚动，页面不应横向溢出。
...
1. 打开左侧“RNG 数据”，点击已导入列表中的“添加数据”；顶部“选择数据”也可打开文件浏览器。在目录列表点“加入”，或点“加入当前文件夹”；也可展开“一次填写多个文件夹路径”，每行填写一个软件运行机器可访问的目录。
...
RNG 数据页面上方的“RNG 数据管理”区域显示当前数据状态与全部已导入引用，未加载时共用列表上的“添加数据”入口；索引准备管理仍按能力显示。可逐项切换或移出列表，不提供对比控件或数量门槛。下方“分析工具”只列物种发现、反应路径和证据核查。从能力提示进入时聚焦相应的准备控件。
